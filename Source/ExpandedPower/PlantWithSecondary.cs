  ...
  public class PlantWithSecondary : Plant {
    ...
    public override void SpawnSetup(Map map, bool respawningAfterLoad) {
      ...
      if (DefDatabase<PlantWithSecondaryDef>.GetNamed(def.defName, false) == null) {
        Log.ErrorOnce("Expanded Power:: Cannot find PlantWithSecondaryDef for " + def.defName + ". Make sure the defNames are the same for both plant and PlantWithSecondaryDef.", 316524160);
        return;
      }
    ...
    public Thing CollectSecondaryThing() {
      ...
      if (!Sec_HarvestableNow) {
        Log.Warning("Expanded Power:: Tried to harvest " + thingLabel + " from " + ThingDef.Named(def.defName).LabelCap + " at " + Position.ToString() + ", but it's not harvestable.");
        return null;
      }
